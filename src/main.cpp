...
void setup() {
  ...
  Serial.println("Running inference mode...");
#endif
}
...
void loop() {
  ...
  if (var > 0.8f) {
    predicted_class = 2; // Fault
  } else if (residual > 0.6f) {
    predicted_class = 1; // Leak
  } else {
  ...
  Serial.print(" | pred="); Serial.println(predicted_class);
#endif
}
